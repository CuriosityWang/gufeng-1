...
define('HS_DB_NAME'    , 'gufeng');     // 数据库名称
define('HS_DB_USER'    , 'root');      // 数据库账号
define('HS_DB_PWD'     , 'root');      // 数据库密码
define('HS_DB_PRE'     , 'gufeng_');    // 数据表统一前缀
...
// 签名验证
function checkSign(){
	if(empty($_POST['sign'])){exit(jsonCode('error', 'sign error1'));}
	$sign = explode('-', $_POST['sign']);
	if(count($sign) != 2){exit(jsonCode('error', 'sign error2'));}
	$db = \hsTool\db::getInstance('access_tokens');
	$token = $db->where('token = ?', array($sign[1]))->fetch();
	if(empty($token)){exit(jsonCode('error', 'sign error3'));}
	$signMd5 = md5($token['token'].$token['time']);
	if($signMd5 != $sign[0]){exit(jsonCode('error', 'sign error4'));}
	// 验证成功则删除
